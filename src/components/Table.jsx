        ...
        <table className="w-full text-sm">
          ...
          {loading ?
            Array.from({ length: 3 }).map((_, rowIndex) => (
              <tbody>
                <tr key={rowIndex} className="h-12 border-b border-white/10">
                  {Array.from({ length: 7 }).map((_, colIndex) => (
                    <td key={colIndex} className="p-4">
                      <div className="w-full h-6 rounded shimmer"></div>
                    </td>
                  ))}
                </tr>
              </tbody>
            )) : !data || data?.length == 0 || data?.watchlist == 0 ?
              <tbody>
                <tr>
                  <td colSpan={7} className="p-6 text-gray-400 text-center text-base">
                  ...
                  return (
                    <tr key={index} className="hover:bg-[#27272a] transition">
                      ...
                      <td className="align-center cursor-pointer relative min-w-36 pr-12">
                        ...
                        {openMenu === index && (
                          <div className="absolute top-[50%] -left-[180%] xl:-left-[150%] 2xl:-left-[130%] -translate-y-1/2 translate-x-full -ml-2 bg-[#27272a] rounded-md shadow-lg z-50">
                            <button
      ...
      {data ?
        ...
        <div className="w-full flex justify-between items-center h-12 bg-[#27272a] text-sm px-6 py-3 rounded-b-lg border border-white/20 border-t-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="h-5 w-4 rounded shimmer " />
