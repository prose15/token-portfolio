    ...
    className="fixed inset-0 flex items-center justify-center bg-black/70 z-50" >
          ...
          className="bg-[#212124] w-1/2 rounded-lg shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="overflow-auto max-h-72 px-4 flex flex-col gap-1">
              ...
              {filtered.map((token, idx) => (
                <button
                  ...
                  className={`flex items-center justify-between w-full px-4 py-3 hover:bg-[#27272a] transition cursor-pointer p-3 rounded-md ${
                    selected.some((t) => t.symbol === token.symbol)? "bg-[#A9E851]/6" : ''}`}
                >
            ...
            <div className="bg-[#27272a] p-2 flex justify-end">
                <button className={`px-3 py-1 rounded-md ${disabled ? 'border border-white/20 cursor-not-allowed' : 'bg-[#A9E851] text-black cursor-pointer'}`} onClick={!disabled && handleSubmit}>
                    Add to WishList
                </button>
            </div>
          </div>
